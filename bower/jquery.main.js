// page init
jQuery(function(){
       $(".slide-block").css('right','-323px');
       
     /*  $(".nav-opener").click(function(){
                          
                          $("#main").toggleClass("active");
                          $(".nav-opener").toggleClass("active");
                          $(".slide-block").toggleClass("active");
                         
                          });*/
       
      /* $(".nav-opener").toggle(function() {
                           $('.slide-block').animate({ right: '0' }, { duration: 400, queue: false });
                           //$('.mobile-menu-bg').animate({ right: '0' }, { duration: 500, queue: false });
                           $('#main').animate({ 'margin-right': '323px' }, { duration: 400, queue: false });
                           $('#main').animate({ 'margin-left': '-323px' }, { duration: 400, queue: false });
                          ...
                          // $('.mobile-menu-bg').animate({ right: '-200' }, { duration: 500, queue: false });
                           $('.slide-block').animate({ right: '-323' }, { duration: 400, queue: false });
                           $('#main').animate({ 'margin-right': '0' }, { duration: 400, queue: false });
                           $('#main').animate({ 'margin-left': '0' }, { duration: 400, queue: false });
                           }
...
/*function initBlocksHeight(){
    ...
    else if(window.location.href.indexOf("callD3-C") > -1) {
        page=".live-callc-page";
    }
   ...
   // alert(page);
	...
	function setHeight(initial){
        ...
        if(page==".coach-details-page" || page==".messages-page")
            addHeight=0;
        ...
        if(page==".archive-detailtext-page" || page==".archive-detailchat-page" || page==".archive-detailcall-page")
			...
			var scrollArea = jQuery(this);
                             ...
                             var top;
           // alert(page+" "+win.height() +" " +scrollArea.offset().top+" "+ footer.outerHeight()+"  "+addHeight);
           // if(scrollArea.offset().top>200)
             //   top=40;
            //else
...
function initOpenClose() {
	...
	var header = jQuery('#header');
     $(".overlay").remove();
	var overlay = jQuery('<span class="overlay"></span>').hide().css({opacity: 0}).appendTo(jQuery('body'));
	var animSpeed = 500;
	var ele;
	var page;
	...
	ele.openClose({
		activeClass: 'active',
		opener: '.nav-opener, .close',
		slider: '.slide-block',
		animSpeed: 400,
		effect: 'slide',
		onInit: function(self){
			self.slider.css('height', win.height() - header.height())
			win.on('resize orientationchange', function(){
				self.slider.css('height', win.height() - header.height())
			});
			
			var firstLevelHolder = self.slider.find('.nav-slide').css({position: 'relative'});
			var levelItems = firstLevelHolder.find('>ul>li:has(.submenu)');
			
			levelItems.each(function(){
				var item = jQuery(this);
				var nextLevelOpener = item.find('>a');
				var secondLevel = item.children('.submenu');
				var backLink = secondLevel.find('.back-menu');
				
				nextLevelOpener.on('click', function(e){
					e.preventDefault();
					secondLevel.show();
					firstLevelHolder.stop().animate({
						marginLeft: '-100%'
					}, animSpeed);
				});
		...
		onBeforeShow: function(self){
			overlay.show().stop().animate({opacity: .75});
		},
...
;(function($) {
  ...
  //alert(self.slider.css("right"));
  if (self.slider.css("right")=="-323px") {
  self.showSlide();
  } else {
  self.hideSlide();
  }
		...
		hideSlide: function() {
			...
			var self = this;
			self.makeCallback('onBeforeClose', self);
			if (self.options.addClassBeforeAnimation) {
	...
	$(function() {
		...
		tabStyleRule += '{position:absolute !important;right: -323px;display:block !important}';
